...
import * as midiDefault from 'midi'
// @ts-ignore
const midi = midiDefault.default

/** @type { ({ name, onMessage }: CreateMidiPortsParams) => MidiPort } */
export function createActualMidiPort({ name, onMessage }) {
  return new MidiPortImpl(name, false, onMessage)
}

/** @type { ({ name, onMessage }: CreateMidiPortsParams) => MidiPort } */
export function createVirtualMidiPort({ name, onMessage }) {
