...
export class LaunchPad {
  ...
  lightPad(note, paletteIndex = 0, pulse = false) {
  ...
  async goCrazy1() {
    ...
    while (Date.now() - start < 1000) {
      for (let note = 11; note <= 99; note++) {
        if (note % 10 === 0) continue;
        this.lightPad(note, randomColor())
      }
      await delay(40)
    }
  ...
  async goCrazy() {
    const randomColor = () => Math.round(Math.random() * 127)
    const randomNote = () => Math.round(Math.random() * 100)
  ...
  scrollText(text, paletteIndex, seconds = 5, speed = DefaultScrollSpeed) {
...
function lightPad(port, note, paletteIndex = 0, pulse = false) {
  const command = pulse ? 0x92 : 0x90
...
function scrollText(port, text = '', paletteIndex = 3, speed = DefaultScrollSpeed) {
  if (text == '') {
    //                  F0h   00h   20h   29h   02h   0Dh   07h   F7h
    port.sendMessage([0xF0, 0x00, 0x20, 0x29, 0x02, 0x0D, 0x07, 0xF7])
    return
  }
